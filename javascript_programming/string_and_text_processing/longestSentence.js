// Write a program that reads the content of a text and then logged out the longest
...
// as a word. You should also log out the number of words in the longest sentence.
//
// Example text:
...
function longestSentence(text) {
  ...
  var maxCount = wordCounts.reduce(function(result, count) {
    return count >= result ? count : result;
  });

  console.log(sentences[wordCounts.indexOf(maxCount)]);
...
function longestSentence(text) {
  ...
  var longest = sentences.reduce(function(result, sentence) {
    return sentence.split(' ').length >= result.split(' ').length ? sentence : result;
  });

  var maxCount = longest.split(' ').length;
